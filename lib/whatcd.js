...
WhatCD.prototype.get_torrent = function get_torrent(parameters, cb) {
  if (!this.logged_in) {
    return this.login(function(err) {
      if (err) {
        return cb(err);
      } else {
        return this.get_torrent(parameters, cb);
      }
    }.bind(this));
  }

  this._request.get({
      url: this.base_url + "/torrents.php?" + querystring.stringify(parameters),
      encoding: null
    ...
    if (!res.headers['content-disposition'])  {
      return cb(Error("No filename information (content-disposition)"));
    }

    var disposition = res.headers['content-disposition'];
    var splits = disposition.split('"');
    if (splits.length != 3) {
      return cb(Error("content-disposition header is unexpected: " + disposition));
    }
    var name = splits[1];

    return cb(err, {filename: name, content: data});
  });
}
